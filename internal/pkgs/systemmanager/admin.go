...
func (svc *AdminService) GetEntry(name string) *types.RegistryEntry {
	return svc.Registry.GetEntryByName(name)
}
...
func (svc *AdminService) StartApplication(name string) (string, error) {
	...
	clientCfg := &types.EndpointConfig{
		Name: regEntry.Endpoint.Name,
		Transport: types.TransportConfig{
			Protocol:  regEntry.Endpoint.Protocol,
			Address:   regEntry.Endpoint.Address,
			Port:      regEntry.Endpoint.Port,
