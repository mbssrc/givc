        ...
        let _async_task = tokio::task::spawn_blocking(|| {
            ...
            if let Ok(true) = systemctl::exists(&service_name) {
                if let Ok(true) = systemctl::is_active(&service_name) {
                    println!(
                        "{service_name} service is running, going to stop {service_name} service now"
                    );
                    let stop_status =
                        systemctl::stop(&service_name).expect("Error in stopping service");
                    if stop_status.success() != true {
                        eprintln!("Cannot stop {service_name} service, {stop_status}");
                    ...
                    println!("Will start copying Journal logs to remote server");
                    ...
                    if start_status.success() {
                        println!("{service_name} service restarted successfully!");
                    } else {
                        eprintln!("Problem in starting back {service_name} service, {start_status}");
                    }
                }
            } else {
                eprintln!("{service_name} service does not exists");
            }
